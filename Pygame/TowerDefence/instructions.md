...
You'll be making a simple tower defence game.

## Game foundations
### Imports and constants setup
```
import pygame
import math
import random
...
## Challenges:
### Tutorial screen
You struggled to know what controls you needed to play the game. Let's fix that by introducing a tutorial screen. We want to put this after all the other draw functions
```
# TUTORIAL SCREEN
def draw_tutorial():
    screen.fill((20, 40, 20))
    ...
    return button
```
We're also going to need to change the main function to accomodate this change.
Find "#tutorial line?" and replace it with:
```
show_tutorial = True
```
And now add the follow bellow
```
        # Show tutorial
        if show_tutorial:
            draw_tutorial()
...
### Colours
Change up some of the colours to make your game unique and yours

### Paving the way
Change up the path the enemies take
