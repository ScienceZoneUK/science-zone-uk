...
Write a program that opens `example.txt` in read mode and prints its contents.

**2. Read Line by Line**
Open `example.txt` and print each line separately.

**3. Count Lines**
Count how many lines are in `example.txt`.

**4. Count Words in File**
Count the total number of words in a file.

**5. Count Characters**
Count the number of characters in a file (including spaces).

**6. Handle Missing File**
Write a program that tries to open `data.txt`. If it doesn’t exist, print `"File not found"`.
...
**9. Append to a File**
Ask the user for a name and append it to `names.txt` without overwriting existing data.

**10. Copy File Contents**
Read from `source.txt` and write the contents into `copy.txt`.
